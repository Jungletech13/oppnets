...
function EditProfileForm({ profile, onSave }: { profile: Profile; onSave: () => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <Field label="Time commitment"><select className="input" value={p.timeCommitment} onChange={(e) => setP({ ...p, timeCommitment: e.target.value as Profile['timeCommitment'] })}><option>Light</option><option>Part-time</option><option>Full-time</option></select></Field>
